...
int SocketManager::SendUDPOnce(const Packet& packet)
{
	// 데이터 보내기   
	int retval = sendto(_send_Socket_UDP, (char*)&packet, sizeof(packet), 0, (SOCKADDR *)&_sendTarget_Addr_UDP, sizeof(_sendTarget_Addr_UDP));
	if (retval == SOCKET_ERROR) { err_display(TEXT("sendto()")); return -1; }
	return 0;
}

int SocketManager::RecvUDPOnce(Packet& packetOutput)
{
	int addrlen;
	int retval;
	...
	SOCKADDR_IN peeraddr;
	addrlen = sizeof(peeraddr);
	retval = recvfrom(_recv_Socket_UDP, (char*)&packetOutput, sizeof(packetOutput), 0, (SOCKADDR *)&peeraddr, &addrlen);
